...
scT1_lwr_in = pd.DataFrame(scT1fd_in.variables['LWRadAtm'][:], columns = ['lwr'])
scT1_swr_in = pd.DataFrame(scT1fd_in.variables['SWRadAtm'][:], columns = ['swr'])
scT1_at_in = pd.DataFrame(scT1fd_in.variables['airtemp'][:], columns = ['at'])
scT1_ap_in = pd.DataFrame(scT1fd_in.variables['airpres'][:], columns = ['ap'])
scT1_ppt_in = pd.DataFrame(scT1fd_in.variables['pptrate'][:], columns = ['ppt'])
scT1_sh_in = pd.DataFrame(scT1fd_in.variables['spechum'][:], columns = ['sh'])
scT1_ws_in = pd.DataFrame(scT1fd_in.variables['windspd'][:], columns = ['ws'])

scT1_allfd_in = pd.concat([scT1_lwr_in , scT1_swr_in, scT1_at_in, scT1_ap_in, scT1_ppt_in, scT1_sh_in, scT1_ws_in], axis=1)
scT1_allfd_in.set_index(pd.DatetimeIndex(scT1date_in),inplace=True)
scT1_allfd_in2014to = scT1_allfd_in['2014-07-01':]

sh_diff =[]
for indx in range (np.size(scT1_sh)):
    sh_diff.append(100*scT1_allfd_in2014to['sh'][indx]/scT1_sh[indx])
...
lat_sa = np.array(scT1fd_in.variables['latitude'][:])
len_lat = np.repeat(lat_sa[:,np.newaxis], hru_num, axis=1); len_lat=len_lat.reshape(hru_num,)

long_sa = np.array(scT1fd_in.variables['longitude'][:])
...
TimeScT1 = scT1fd_in.variables['time'][32184:] # get values
...
#humidity1 = testfd1.variables['spechum'][:,0]
fsh = testfd.variables['LWRadAtm'][:,0]
